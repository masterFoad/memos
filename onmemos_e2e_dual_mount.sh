...
# OnMemOS E2E dual-mount validator (GCS Fuse + Filestore) on GKE Autopilot
# - Ensures namespace, KSA, GSA, Workload Identity binding
# - Ensures/creates a bucket and grants bucket IAM to the workspace GSA
# - Resolves Filestore IP (from instance+zone or direct IP)
# - Creates static PV (NFS) + PVC (storageClassName: ""; volumeName bound)
# - Deploys a pod that mounts both /workspace (bucket) and /data (Filestore)
# - Polls until PodScheduled and Ready; prints full diagnostics on failure
# - Optional cleanup
#
# Usage example:
#   ./onmemos_e2e_dual_mount.sh \
...
#     --location us-central1 --wait-seconds 900 --cleanup
...
BUCKET=""; LOCATION="us-central1"; WAIT_S=900; CLEANUP=false
...
Usage:
  $0 --project <id> --region <region> --cluster <name> --workspace-id <id> \
     ...
     [--pvc-size 100Gi] [--wait-seconds 900] [--cleanup]
EOF
      exit 0;;
    *) die "unknown flag: $1";;
  esac
...
log "Create PV + PVC (Filestore CSI static bind, StorageClass disabled)"
...
log "Apply dual-mount pod (PVC + GCS Fuse)"
cat <<YAML | kubectl -n "$NS" apply -f -
apiVersion: v1
kind: Pod
metadata:
...
spec:
  ...
  - name: filestore
    persistentVolumeClaim:
      claimName: $PVC_NAME
    # mount a per-workspace subdirectory under the share (optional)
...
kubectl -n "$NS" wait --for=condition=Ready --timeout=${WAIT_S}s pod "$POD_NAME" || {
  ...
  echo "- Filestore path: $FS_IP:$EXPORT_PATH reachable from cluster VPC"
  exit 3
}
...
if $CLEANUP; then
  log "Cleanup: delete pod (PV/PVC retained for reuse)"
