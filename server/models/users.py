...
# User entitlements configuration
USER_ENTITLEMENTS = {
...
class UserManager:
    """Manages user profiles and workspace entitlements"""
    
    def __init__(self):
        self.users: Dict[str, UserProfile] = {}
    
    def create_user(self, user_id: str, user_type: UserType = UserType.FREE, 
                   email: Optional[str] = None, name: Optional[str] = None) -> UserProfile:
    ...
    def create_workspace(self, user_id: str, workspace_id: str, name: str, 
                        ...
                        description: Optional[str] = None) -> WorkspaceProfile:
    ...
    def can_allocate_storage(self, workspace_id: str, request: WorkspaceStorageRequest) -> bool:
        ...
        # Check bucket limits (workspace can have multiple buckets)
        if request.request_bucket:
            # For now, allow multiple buckets per workspace
            pass
        
        # Check persistent storage limits (workspace can have multiple PVCs)
        if request.request_persistent_storage:
            # For now, allow multiple PVCs per workspace
            pass
        
        # Check total storage size (implement workspace-specific limits later)
        requested_size = (request.bucket_size_gb or 0) + (request.persistent_storage_size_gb or 0)
        # TODO: Implement workspace storage limits based on resource package
    ...
    def allocate_storage(self, workspace_id: str, request: WorkspaceStorageRequest) -> WorkspaceStorageAllocation:
        ...
        if request.request_bucket:
            bucket_name = request.bucket_name or f"onmemos-{workspace_id}-{request.namespace}-{timestamp}"
            allocation.bucket_name = bucket_name
            workspace.current_buckets.append(bucket_name)
        
        if request.request_persistent_storage:
            pvc_name = f"pvc-{workspace_id}-{request.namespace}-{timestamp}"
            allocation.persistent_volume_name = pvc_name
            workspace.current_pvcs.append(pvc_name)
        
        # Update storage usage
        allocation.storage_size_gb = (request.bucket_size_gb or 0) + (request.persistent_storage_size_gb or 0)
    ...
    def deallocate_storage(self, workspace_id: str, allocation: WorkspaceStorageAllocation):
        ...
        # Update storage usage
        workspace.current_storage_gb -= allocation.storage_size_gb
        workspace.current_storage_gb = max(0, workspace.current_storage_gb)
